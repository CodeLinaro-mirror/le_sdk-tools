...
#   ${1} - (mandatory) path to container config json
function qml-sync-python-wrapper() {
    ...
    (
        ...
        adb push ${QML_DOCKER_DIR}/src/python/ /tmp/

        rc=$?
        [ $rc -ne 0 ] && {
            print-red "FAILED: adb push ${QML_DOCKER_DIR}/src/python/ /tmp/ !!!"
            return $rc
        }

        qml-device-command "docker cp /tmp/python ${QML_CONTAINER_NAME}:${QML_ARG_BASE_DIR}/src/" ${DEVICE_ID}

        rc=$?
        [ $rc -ne 0 ] && {
            print-red "FAILED: qml-device-command !!!"
            qml-device-command "rm -rf /tmp/python" ${DEVICE_ID}
...
function qml-run-python-wrapper() {
    ...
    (
        ...
        time adb shell "docker exec ${QML_CONTAINER_NAME}                                          \
            python3 ${QML_ARG_BASE_DIR}/src/python/snpe/test_snpe/snpe_main.py                     \
                -d ${QML_ARG_BASE_DIR}/examples/${MODEL}                                           \
                -i ${QML_ARG_BASE_DIR}/examples/inputs.txt                                         \
                -r ${BACKEND_lc}                                                                   \
                -o ${QML_ARG_BASE_DIR}/${TEST_OUTPUT_DIRECTORY_NAME}                               \
                -b ${BUFFER_uc}                                                                    \
...
print-blue "qml-sync-python-wrapper                                   <path/to/targets/dev_config.json>"
echo "    Sync Python wrapper src and test examples to the container"
print-blue "qml-run-python-wrapper                                    <path/to/targets/dev_config.json>"
