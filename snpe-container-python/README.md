...
1. config.json contains fields required to build, update and run SNPE container from host to the device.
2. dev_config.json contains fields required to sync models and inputs from host to the device. This is useful to run the models using python wrapper into the SNPE container.
...
The functions in ```/scripts/host/docker_env_setup.sh``` provide the necessary docker build, load, run, start, stop and remove commands. All of these receive the path to the configuration json file as their first and only argument, as shown in the examples bellow
...
These functions are used for providing python wrapper scripts to SNPE tools.
...
- qml-sync-snpe - Sync Python wrapper src and test examples to the container
- qml-run-snpe - Run Python wrapper script on the container
