...
function qairt-device-prepare() {
    ...
    (
        ...
        qairt-device-command "mount -o remount,rw / > /dev/null" ${DEVICE_ID}
        rc=$?
        [ "${rc}" -ne 0 ] && print-red "adb file system remount failed !!!" && return -3

        qairt-device-command "! command -v setenforce || setenforce 0" ${DEVICE_ID}
        rc=$?
...
function qairt-get-container-and-image-name() {
    local PATH_TO_CONFIG_JSON=$1
    local -n OUT_QAIRT_CONTAINER_NAME=$2
    local -n OUT_QAIRT_IMAGE_NAME=$3
    ...
    local QAIRT_ADDITIONAL_TAG=$(echo ${JSON_CONTENT} |  jq '.Additional_tag_container' | tr -d '"')

    [ ! -z "${QAIRT_ADDITIONAL_TAG}" ] && {
        QAIRT_ADDITIONAL_TAG="-${QAIRT_ADDITIONAL_TAG}"
    }

    OUT_QAIRT_CONTAINER_NAME="qairt${QAIRT_ADDITIONAL_TAG}"
