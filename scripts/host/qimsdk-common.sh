    ...
    [ "${QIMSDK_ESDK_TFLITE_FILENAME}" != "no-tflite-dev-archive-available" ]                   && \
        ( ln ${QIMSDK_ESDK_TFLITE_FILE} ${QIMSDK_ESDK_BASE_DIR}/downloads/ 2>/dev/null          || \
        ...
        )                                                                                       || \
            {
                ...
                touch ${QIMSDK_ESDK_BASE_DIR}/${QIMSDK_ARG_TFLITE_FILENAME}
            }
...
# Setup snpe
function qimsdk-setup-snpe() {
    local SNPE_DIR=`echo ${BUFFER} |  jq '.SNPE_path' | tr -d '"'`

    [ -d "${SNPE_DIR}" ]                                                                        && \
        QIMSDK_ESDK_SNPE_DIR=snpe                                                               && \
            ( rsync -a ${SNPE_DIR}/* ${QIMSDK_ESDK_BASE_DIR}/downloads/${QIMSDK_ESDK_SNPE_DIR} || \
                {
                    print-red "Cannot add snpe dir to esdk base folder !!!"
                    rm -rf ${QIMSDK_ESDK_BASE_DIR}
                    return -9
                }
              [ -f ${QIMSDK_ESDK_BASE_DIR}/downloads/${QIMSDK_ESDK_SNPE_DIR}/lib/aarch64-oe-linux-gcc8.2/libatomic.so.1 ] && \
                rm -f ${QIMSDK_ESDK_BASE_DIR}/downloads/${QIMSDK_ESDK_SNPE_DIR}/lib/aarch64-oe-linux-gcc8.2/libatomic.so.1
            )                                                                                   || \
                {
                    QIMSDK_ESDK_SNPE_DIR=no-snpe-dir-available
                    touch ${QIMSDK_ESDK_BASE_DIR}/${QIMSDK_ESDK_SNPE_DIR}
                }
    return 0
}

# Remove snpe
function qimsdk-remove-snpe() {
    [ -d "${SNPE_DIR}" ]                                                                     && \
        rm -rf ${QIMSDK_ESDK_BASE_DIR}/downloads/${QIMSDK_ESDK_SNPE_DIR}
