...
function qimsdk-target-get-updated-packages-all() {
    ...
    done < <(find ${QIMSDK_ESDK_BASE_DIR}/tmp/work/aarch64-oe-linux/ubuntu-base/20.04-r0/ubuntu_base_tmp/var/cache/apt/archives ${QIMSDK_ESDK_BASE_DIR}/tmp/deploy/${PKG_FORMAT}/ -type f \( -name "*gstreamer*.${PKG_FORMAT}" -o -name "libgst*.${PKG_FORMAT}" -o -name "*liborc*.${PKG_FORMAT}" -o -name "*libdaemon*.${PKG_FORMAT}" -o -name "*libgudev*.${PKG_FORMAT}" -o -name "*libjson-glib*.${PKG_FORMAT}" -o -name "*libmp3lame*.${PKG_FORMAT}" -o -name "*libpsl*.${PKG_FORMAT}" -o -name "*libtheora*.${PKG_FORMAT}" -o -name "*libtag*.${PKG_FORMAT}" -o -name "*libsoup*.${PKG_FORMAT}" -o -name "*libspeex*.${PKG_FORMAT}" -o -name "mpg123*.${PKG_FORMAT}" \) -print0 )
    while IFS= read -r -d $'\0'; do
        UPDATED_PACKAGES+=("$REPLY")
...
function qimsdk-target-get-updated-packages-rel() {
    ...
    while IFS= read -r -d $'\0'; do
        UPDATED_PACKAGES+=("$REPLY")
    done < <(find ${QIMSDK_ESDK_BASE_DIR}/tmp/work/aarch64-oe-linux/ubuntu-base/20.04-r0/ubuntu_base_tmp/var/cache/apt/archives ${QIMSDK_ESDK_BASE_DIR}/tmp/deploy/${PKG_FORMAT}/ -type f \( -name "*.${PKG_FORMAT}" ! -iname "qnn*.${PKG_FORMAT}" ! -iname "snpe*.${PKG_FORMAT}" ! -iname  "*gstreamer*.${PKG_FORMAT}" ! -iname "*-dev_*" ! -iname "*-staticdev_*" ! -iname "*-doc_*" ! -iname "*-dbg_*" ! -iname "*-locale-*" \) -cnewer ${QIMSDK_WORK_DIR}/prepared -print0)
}
...
function qimsdk-target-get-updated-packages-dbg() {
    ...
    while IFS= read -r -d $'\0'; do
        UPDATED_PACKAGES+=("$REPLY")
    done < <(find ${QIMSDK_ESDK_BASE_DIR}/tmp/work/aarch64-oe-linux/ubuntu-base/20.04-r0/ubuntu_base_tmp/var/cache/apt/archives ${QIMSDK_ESDK_BASE_DIR}/tmp/deploy/${PKG_FORMAT}/ -type f \( -name "*gstreamer*-dbg_*.${PKG_FORMAT}" ! -iname "*gstreamer*-dev_*" ! -iname "*gstreamer*-staticdev_*" ! -iname "*gstreamer*-doc_*" -o -name "*libdaemon*-dbg_*.${PKG_FORMAT}" -o -name "*libgudev*-dbg_*.${PKG_FORMAT}" -o -name "*libjson-glib*-dbg_*.${PKG_FORMAT}" -o -name "*libpsl*-dbg_*.${PKG_FORMAT}" -o -name "*libtheora*-dbg_*.${PKG_FORMAT}" -o -name "*libtag*-dbg_*.${PKG_FORMAT}" -o -name "*libsoup*-dbg_*.${PKG_FORMAT}" -o -name "*libspeex*-dbg_*.${PKG_FORMAT}" -o -name "mpg123*-dbg_*.${PKG_FORMAT}" \) -print0 )
    while IFS= read -r -d $'\0'; do
        UPDATED_PACKAGES+=("$REPLY")
    done < <(find ${QIMSDK_ESDK_BASE_DIR}/tmp/work/aarch64-oe-linux/ubuntu-base/20.04-r0/ubuntu_base_tmp/var/cache/apt/archives ${QIMSDK_ESDK_BASE_DIR}/tmp/deploy/${PKG_FORMAT}/ -type f \( -name "*-dbg_*.${PKG_FORMAT}" ! -iname "qnn*.${PKG_FORMAT}" ! -iname "snpe*-dbg_*.${PKG_FORMAT}" ! -iname  "*gstreamer*.${PKG_FORMAT}" ! -iname "*-dev_*" ! -iname "*-staticdev_*" ! -iname "*-doc_*" \) -cnewer ${QIMSDK_WORK_DIR}/prepared -print0)
}
...
function qimsdk-target-get-updated-packages-dev() {
    ...
    while IFS= read -r -d $'\0'; do
        UPDATED_PACKAGES+=("$REPLY")
    done < <(find ${QIMSDK_ESDK_BASE_DIR}/tmp/work/aarch64-oe-linux/ubuntu-base/20.04-r0/ubuntu_base_tmp/var/cache/apt/archives ${QIMSDK_ESDK_BASE_DIR}/tmp/deploy/${PKG_FORMAT}/ -type f \( -name "*gstreamer*-dev_*.${PKG_FORMAT}" ! -iname "*gstreamer*-staticdev_*" ! -iname "*gstreamer*-doc_*" ! -iname "*gstreamer*-dbg_*" -o -name "*libdaemon*-dev_*.${PKG_FORMAT}" -o -name "*libgudev*-dev_*.${PKG_FORMAT}" -o -name "*libjson-glib*-dev_*.${PKG_FORMAT}" -o -name "*libmp3lame-dev_**.${PKG_FORMAT}" -o -name "*libpsl*-dev_*.${PKG_FORMAT}" -o -name "*libtheora*-dev_*.${PKG_FORMAT}" -o -name "*libtag*-dev_*.${PKG_FORMAT}" -o -name "*libsoup*-dev_*.${PKG_FORMAT}" -o -name "*libspeex*-dev_*.${PKG_FORMAT}" -o -name "mpg123*-dev_*.${PKG_FORMAT}" \) -print0 )
    while IFS= read -r -d $'\0'; do
        UPDATED_PACKAGES+=("$REPLY")
    done < <(find ${QIMSDK_ESDK_BASE_DIR}/tmp/work/aarch64-oe-linux/ubuntu-base/20.04-r0/ubuntu_base_tmp/var/cache/apt/archives ${QIMSDK_ESDK_BASE_DIR}/tmp/deploy/${PKG_FORMAT}/ -type f \( -name "*-dev_*.${PKG_FORMAT}" ! -iname "qnn*.${PKG_FORMAT}" ! -iname "snpe*-dev_*.${PKG_FORMAT}" ! -iname  "*gstreamer*.${PKG_FORMAT}" ! -iname "*-dbg_*" ! -iname "*-staticdev_*" ! -iname "*-doc_*" \) -cnewer ${QIMSDK_WORK_DIR}/prepared -print0)
}
