...
function qimsdk-target-get-updated-packages-rel() {
    ...
    while IFS= read -r -d $'\0'; do
        UPDATED_PACKAGES+=("$REPLY")
    done < <(find ${QIMSDK_ESDK_BASE_DIR}/tmp/work/aarch64-oe-linux/ubuntu-base/20.04-r0/ubuntu_base_tmp/var/cache/apt/archives ${QIMSDK_ESDK_BASE_DIR}/tmp/deploy/${PKG_FORMAT}/ -type f \( -name "*.${PKG_FORMAT}" ! -iname "snpe*.${PKG_FORMAT}" ! -iname  "*gstreamer*.${PKG_FORMAT}" ! -iname "*-dev_*" ! -iname "*-staticdev_*" ! -iname "*-doc_*" ! -iname "*-dbg_*" ! -iname "*-locale-*" \) -cnewer ${QIMSDK_WORK_DIR}/prepared -print0)
}
