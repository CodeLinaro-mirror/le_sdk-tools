...
function qimsdk-target-get-updated-packages-all() {
    ...
    done < <(find ${QIMSDK_ESDK_BASE_DIR}/tmp/work/aarch64-oe-linux/ubuntu-base/20.04-r0/ubuntu_base_tmp/var/cache/apt/archives ${QIMSDK_ESDK_BASE_DIR}/tmp/deploy/${PKG_FORMAT}/ -type f \( -name "*gstreamer*.${PKG_FORMAT}" -o -name "libgst*.${PKG_FORMAT}" -o -name "*liborc*.${PKG_FORMAT}" -o -name "*libdaemon*.${PKG_FORMAT}" -o -name "*libgudev*.${PKG_FORMAT}" -o -name "*libjson-glib*.${PKG_FORMAT}" -o -name "*libmp3lame*.${PKG_FORMAT}" -o -name "*libpsl*.${PKG_FORMAT}" -o -name "*libtheora*.${PKG_FORMAT}" -o -name "*libtag*.${PKG_FORMAT}" -o -name "*libsoup*.${PKG_FORMAT}" -o -name "*libspeex1*.${PKG_FORMAT}" -o -name "*gobject-introspection*.${PKG_FORMAT}" -o -name "mpg123*.${PKG_FORMAT}" \) -print0 )
    while IFS= read -r -d $'\0'; do
        UPDATED_PACKAGES+=("$REPLY")
