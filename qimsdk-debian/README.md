...
* [Scripts](#Scripts)
    ...
    * [RecipeParser.py](#RecipeParser.py)
    * [setup.sh](#setup.sh)
* [Workflow](#Workflow)
...
#### qimsdk-build
    ...
    13. Invoke Recipe Parser script to:
        13.1 get list of patches to be applied to OS projects
        13.2 get list of qti plugins that need to be built and generate build script
    ...
    16. Call incremental build function from previously generated build script which builds open-source and qti GStreamer plugins
...
- qimsdk-incremental-build - Main entry point that builds all GStreamer components in sequence with success reporting. Also calls qimsdk-incremental-build-qti, generated by RecipeParser.py to build QTI GStreamer plugins.
...
<div id="RecipeParser.py">

#### RecipeParser.py

Automatically generates shell scripts for building GStreamer plugins in Qualcomm's SDK environment. It parses QTI Gstreamer meta (.bb files in meta-qti-gst layer fetched in Build Image code), extracts plugin information, and creates optimized build scripts with parallelization capabilities.

- Parsable - Abstract base class for parsing operations with common path setup and helper methods
  - _parse_helper (Parsable) - Helper that creates temporary files for intermediate parsing operations
- RecipeParser - Subclass that initializes and collects GStreamer recipe files from meta layers
- BuildCodeGenerator - Main implementation that processes recipes and generates build scripts
  - init (BuildCodeGenerator) - Sets up files to parse, extracts plugin list from packagegroups, applies filtering
  - process (BuildCodeGenerator) - Extracts source paths and package summaries from QTI Gstreamer meta
  - export (BuildCodeGenerator) - Generates shell script with build/clean functions and parallel build logic for QTI GStreamer plugins
- parse_arguments - Parses command-line inputs for paths and actions
- main - Entry point: parses args, creates parser, processes and exports

The script specifically:

    1. Locates GStreamer recipe files in meta layers
    2. Identifies which plugins to build by parsing RDEPENDS declarations
    3. Extracts source paths and metadata from each plugin's recipe
    4. Generates a shell function(qimsdk-incremental-build-qti) with:
        - Individual build/clean functions per plugin
        - Parallelized incremental build capability
        - Help documentation for available commands
    5. Optimizes build order with base plugins built first, followed by dependent ones in parallel batches
...
- qimsdk-invoke-recipe-parser - Invokes Python-based recipe parser(RecipeParser.py) and provides the Necessary arguments for it
