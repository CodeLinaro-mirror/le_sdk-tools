...
* 8. ***OPTIONAL*** - **Deploy_QIMSDK_Artifacts_tag** - Appends value in this field to artifacts archive name, e.g. if value is "1.0.0" result would be packages_1.0.0.zip (if QIMSDK artifacts do not needed suffix, just leave the value for this field empty)
* 9. ***MANDATORY*** - **Gst_package_group** - Gst package group (packagegroup-qti-gst-basic or packagegroup-qti-gst)
...
* 11. ***MANDATORY*** - **Device_install_prefix** - Prefix to be prepended to the package installation path on target device - Installing packages in / directory is forbidden. Do not leave field empty, or set / value !!! (e.g. choosing /data/sdk would install gst libs in /data/sdk/usr/lib/gstreamer-1.0)
