...
Config json files *(config.json)* must contain the following data:
 ...
 10. ***MANDATORY*** - **Path_to_headers_dir** - Path to usr/include headers dir, containing all needed headers for qimsdk-build image compilation. ***Note: Custom usr/include/ dir can be provided, but subfolder ./sysroots/\<arch-name\>/usr/include/ of unarchived standard SDK contains the needed headers.***
 11. ***MANDATORY*** - **Path_to_pkgconfig_dir** - Path to usr/lib/pkgconfig packageconfig dir, containing all needed pkg-config files for qimsdk-build image compilation. ***Note: Custom usr/lib/pkgconfig/ dir can be provided, but subfolder ./sysroots/\<arch-name\>/usr/lib/pkgconfig/ of unarchived standard SDK contains the needed pkg-config files.***
...
- qimsdk-docker-device-save-image      \<path-to-config-json> - Save selected device image, compose file and run command
...
To run the container in CDI mode **qimsdk-docker-device-run-cdi-container** command should be invoked.
```bash
...
```bash
docker-compose -f <docker-compose-cdi.yml> up -d
```
...
***IMPORTANT: Standard SDK is used to get include heaeders and pkg-config files needed for qimsdk-build image compilation. Custom usr/lib/pkgconfig/ and usr/include dirs can be provided, if user does not wish to use Standard SDK. This is done by copying custom header and pkg-config directory contents to respective destinations like so:***
  ...
  - Instructions how to set up projects to be built inside QIMSDK Development container:
    ...
    <ul>
      ...
      ```bash
      # Example line to send synced project to temporary directory to be used by dev container:
      ln -s <path/to/sources>/solutions-microservices/microservices/qimsdk <current/docker/dir>/tmp/solutions-microservices
      ```
    ...
    <ul>
      <div style="color:#90EE90">DIR: tmp/patches</div>
      content of patches directory:

      <div name="gstreamer1.0">gstreamer-1.24.2</div>

      ```bash
        rsync -a <path/to/unarchived/SDK/directory>/layers/meta-qti-gst/recipes-gst/gstreamer/gstreamer1.0/1.24/*.patch \
          <current/docker/dir>/tmp/patches/gstreamer-1.24.2/
      ```

      <div name="gst-plugins-base">gst-plugins-base-1.24.2</div>

      ```bash
        rsync -a <path/to/unarchived/SDK/directory>/layers/meta-qti-gst/recipes-gst/gstreamer/gstreamer1.0-plugins-base/1.24/*.patch \
      ...
      ```bash
        rsync -a <path/to/unarchived/SDK/directory>/layers/meta-qti-gst/recipes-gst/gstreamer/gstreamer1.0-plugins-good/1.24.2/*.patch \
          <current/docker/dir>/tmp/patches/gst-plugins-good-1.24.2/
      ```

      <div name="gst-plugins-bad">gst-plugins-bad-1.24.2</div>

      ```bash
        rsync -a <path/to/unarchived/SDK/directory>/layers/meta-qti-gst/recipes-gst/gstreamer/gstreamer1.0-plugins-bad/1.24.2/*.patch \
          <current/docker/dir>/tmp/patches/gst-plugins-bad-1.24.2/
      ```
    </ul>
...
### Docker Build
  ...
  <div style="color:#90EE90">QNP version</div>
    ...
    ENV QIMSDK_QNP_VER=2.34.0.250424

  but it can be modified as

    ENV QIMSDK_QNP_SDK=<major version>.<minor version>.<patch version>.<YY><MM><DD>
...
<h3 style="color:red">
  <b>
  Please note that QIMSDK Container uses CDI,
  so CDI needs to be enabled in target Docker configuration
  </b>
</h3>
...
- In order to use this functionality, there are some requirements:
  - sdk-tools/qimsdk directory is needed in Build machine, where QIMSDK device image is generated.
  - sdk-tools/qimsdk directory is needed in Remote PC, connected to device.
